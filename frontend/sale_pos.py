...
class SalePOS:
    ...
    def add_to_cart(self, product_id, product):
        try:
            ...
            # ✅ ADD purchase_price to cart item
            cart_item = {
                ...
                'purchase_price': float(purchase_price),  # ✅ ADD THIS LINE
                'quantity': 1,
    ...
    def get_product_details_display(self, item):
        """Get formatted product details for display in cart and invoice"""
        try:
            category = item.get('category_name', '').lower()
            details_parts = []
            
            # Add color if available
            if item.get('color') and str(item['color']).strip() and item['color'] != 'N/A':
                details_parts.append(f"Color: {item['color']}")
            
            # Category-specific details
            if 'paint' in category:
                if item.get('packing') and str(item['packing']).strip() and item['packing'] != 'N/A':
                    details_parts.append(f"Packing: {item['packing']}")
            elif 'roof sheet' in category or 'limination sheet' in category or 'sanitary' in category:
                if item.get('volume') and str(item['volume']).strip() and item['volume'] != 'N/A':
                    details_parts.append(f"Size: {item['volume']}")
            else:
                # Default for hardware and other categories
                if item.get('packing') and str(item['packing']).strip() and item['packing'] != 'N/A':
                    details_parts.append(f"Packing: {item['packing']}")
                if item.get('volume') and str(item['volume']).strip() and item['volume'] != 'N/A':
                    details_parts.append(f"Size: {item['volume']}")
            
            return '\n'.join(details_parts) if details_parts else "Standard"
            
        except Exception as e:
            print(f"Error in get_product_details_display: {e}")
            ...
            if customer_id is None and customer_name != "Walk-in Customer":
                try:
                    customer_id = self.sale_service.add_customer(
                        name=customer_name,
                        phone="00000000000",  # Default phone
                        address="Not specified"
                    )
                except Exception as e:
                    print(f"Failed to create customer: {e}")
                    customer_id = 1  # Use Walk-in Customer as fallback

            # Use Walk-in Customer if no customer found/created
            if customer_id is None:
                customer_id = 1
                
            # ✅ FIXED: Call create_sale with correct number of arguments
            sale_id = self.sale_service.create_sale(
                customer_id=customer_id,
                total_amount=subtotal,
                discount=discount,
                final_amount=total
                # payment_method='cash' is now the default, so we don't need to pass it
            )
