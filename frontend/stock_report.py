...
#     def is_roof_sheet_in_feet(self, category_name, packing):
#         """Check if the product is a Roof Sheet measured in feet"""
#         if not category_name:
#             return False
        
#         category_lower = category_name.lower()
#         packing_lower = str(packing).lower() if packing else ""
        
#         # Check if it's Roof Sheet category and packing indicates feet measurement
#         return ('roof sheet' in category_lower and 
#                 ('feet' in packing_lower or 'ft' in packing_lower))
...
#                         # Check if this is a Roof Sheet product stored in feet
#                         is_roof_sheet_feet = self.is_roof_sheet_in_feet(category_name, packing)
                        
#                         # Stock display - show feet for feet-based Roof Sheets
#                         if is_roof_sheet_feet:
#                             stock_display = f"{current_stock} ft"
#                         else:
#                             stock_display = f"{current_stock}"
                        
#                         # Stock value
#                         stock_value = current_stock * purchase_price
                        
#                         # Status with better formatting - adjust thresholds for feet
#                         if current_stock == 0:
#                             status = "❌ Out of Stock"
#                         elif is_roof_sheet_feet:
#                             # Different thresholds for feet-based products
#                             if current_stock <= 10:  # 10 feet is critical for Roof Sheets
#                                 status = "🔴 Critical"
#                             elif current_stock <= 25:  # 25 feet is low stock
#                                 status = "🟡 Low Stock"
#                             else:
#                                 status = "✅ In Stock"
#                         else:
#                             # Regular thresholds for units-based products
#                             if current_stock <= 2:
#                                 status = "🔴 Critical"
...
#                             product_name,
#                             category_name,
#                             stock_display,  # This now shows units or feet
#                             f"₨{purchase_price:,.0f}",
#                             f"₨{stock_value:,.0f}",
#                             status
#                         ))
...
#                             # Check if this is a Roof Sheet product stored in feet
#                             is_roof_sheet_feet = self.is_roof_sheet_in_feet(cat_name, packing)
...
#                             # Status determination with appropriate thresholds
#                             if current_stock == 0:
#                                 status = "Out of Stock"
#                             elif is_roof_sheet_feet:
#                                 if current_stock <= 10:
...
class StockReport:
    ...
    def get_measurement_display(self, product):
        """Get measurement display for a product"""
        try:
            if len(product) >= 14:
                packing = product[7]  # packing field
                current_stock = product[9]  # current_stock
                category_name = product[13]  # category_name
                
                # Use the stock service to get measurement type and format display
                measurement_type = self.stock_service.get_measurement_type_from_packing(packing, category_name)
                return self.stock_service.format_stock_display(current_stock, measurement_type)
            return str(product[9]) if len(product) > 9 else "0"
        except:
            return str(product[9]) if len(product) > 9 else "0"

    def get_product_status(self, product):
        """Get status for a product with measurement-aware thresholds"""
        try:
            if len(product) >= 14:
                packing = product[7]  # packing field
                current_stock = product[9]  # current_stock
                ...
                # Use the stock service to get status
                measurement_type = self.stock_service.get_measurement_type_from_packing(packing, category_name)
                return self.stock_service.get_stock_status(current_stock, measurement_type)
            return "❌ Out of Stock"
        except:
            return "❌ Error"
            ...
            for product in products:
                try:
                    if len(product) >= 14:
                        ...
                        # Get stock display with proper measurement unit
                        stock_display = self.get_measurement_display(product)
                        
                        # Stock value
                        stock_value = current_stock * purchase_price
                        
                        # Get status with measurement-aware thresholds
                        ...
                        self.products_tree.insert('', 'end', values=(
                            product_name,
                            category_name,
                            stock_display,  # This now shows proper units (ft, m, kg, L, pcs)
                            f"₨{purchase_price:,.0f}",
                            f"₨{stock_value:,.0f}",
                            status
                        ))
                ...
                for category_name, products in categories_with_low_stock.items():
                    ...
                    for product in products:
                        if len(product) >= 14:
                            ...
                            # Get measurement type and display
                            measurement_type = self.stock_service.get_measurement_type_from_packing(packing, cat_name)
                            stock_display = self.stock_service.format_stock_display(current_stock, measurement_type)
                            
                            stock_value = current_stock * purchase_price
                            
                            # Get status with measurement-aware thresholds
                            status_text = self.stock_service.get_stock_status(current_stock, measurement_type)
                            # Remove emojis for PDF
                            status = status_text.replace("❌ ", "").replace("🔴 ", "").replace("🟡 ", "").replace("✅ ", "")
                            
                            category_data.append([
                                product_name,
                                stock_display,  # This now shows proper units
                                f"₨{purchase_price:,.0f}",
                                f"₨{stock_value:,.0f}",
                                status
                            ])
